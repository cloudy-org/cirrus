...
impl<'a, T: CConfig> ConfigManager<T> {
    pub fn new(app_name: &str, template_config_toml_string: &'a str) -> Result<Self, Box<dyn CError>> {
        let config = Self::get_or_create_config_file(app_name, template_config_toml_string)?;
    ...
    fn get_or_create_config_file(app_name: &str, template_config_toml_string: &'a str) -> Result<T, Box<dyn CError>> {
